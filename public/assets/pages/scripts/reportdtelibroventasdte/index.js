...
function consultarpage(aux_data){
    ...
    $('#tabla-data-consulta').DataTable({
        ...
        'serverSide'  : true,
        "order"       : [[ 0, "asc" ]],
        'ajax'        : "/reportdtelibroventasdte/reportdtelibroventasdtepage/" + aux_data.data2, //$("#annomes").val() + "/sucursal/" + $("#sucursal_id").val(),
        'columns'     : [
            {data: 'id'}, // 0
            {data: 'fechahora'}, // 1
            {data: 'rut'}, // 2
...
function totalizar(){
    ...
    data = datosFac();
    $.ajax({
        url: '/reportdtelibroventasdte/totalizarindex/' + data.data2,
        type: 'GET',
...
function btnpdf(data){
    //console.log(data);
    //alert('entro');
    $('#contpdf').attr('src', '/reportdtelibroventasdte/exportPdf/'+data.data2);
