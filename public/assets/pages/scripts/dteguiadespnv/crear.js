...
function sumcant(){
	...
	aux_totalNeto = 0;
	aux_totalKg = 0;
	$("#tabla-data tr .cantsum").each(function() {
		valor = $(this).val();
		valorNum = parseFloat(valor);
		aux_fila = $(this).attr("fila");
		if(isNaN( valorNum )){
			valorNum = 0;
		}
		aux_cant_valor = parseFloat($(this).attr("valor"));
		if(valorNum < 0 || valorNum > aux_cant_valor){
		...
		$(this).val(valorNum);
		aux_prcitem = $("#prcitem" + aux_fila).val();
		aux_subtotal = valorNum * parseFloat(aux_prcitem);
		$("#lblsubtotal" + aux_fila).html(aux_subtotal);
		$("#subtotal" + aux_fila).val(aux_subtotal);
		$("#montoitem" + aux_fila).val(aux_subtotal);
		aux_kg_valor = Math.floor($("#totalkilos" + aux_fila).attr("valor"));
		if(aux_kg_valor > 0){
			aux_UnitKg = aux_kg_valor / aux_cant_valor;
		}else{
			aux_UnitKg = 0;
		}
		aux_subtotalKg = aux_UnitKg * valorNum;
		$("#aux_kilos" + aux_fila).html(aux_subtotalKg);
		$("#totalkilos" + aux_fila).val(aux_subtotalKg);
		$("#itemkg" + aux_fila).val(aux_subtotalKg);
		aux_totalKg += aux_subtotalKg;

		aux_total += valorNum;
		aux_totalNeto += aux_subtotal;
	});
	if(aux_total==0){
		$("#cantTotal").val("");
		$("#total").val("");
	}else{
		$("#cantTotal").val(aux_total);
		$("#total").val(aux_totalNeto);
	}
	$("#totalkg").html(aux_totalKg);
	$("#tdneto").html(MASKLA(aux_totalNeto,0));
	aux_tasaiva = $("#tasaiva").val();
	aux_tasaiva = parseFloat(aux_tasaiva);

	aux_Tiva = Math.round(aux_tasaiva * aux_totalNeto/100);
	aux_total = aux_totalNeto + aux_Tiva;
	$("#tdiva").html(MASKLA(aux_Tiva,0));
	$("#tdtotal").html(MASKLA(aux_total,0));
}