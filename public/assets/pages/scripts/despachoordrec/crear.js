...
$("#documento_file").on("change", function(){
	$("#imagen").val($('#documento_file').val());
	//$('#documento_file').attr('valor');
});

$("#form-general").on('submit', function (event) {
	aux_id = $('#documento_id').val();
	aux_file = $('#imagen').val();
	if( aux_id == null || aux_id.length == 0 || /^\s+$/.test(aux_id) ){
		aux_id = "";
	}
	if( aux_file == null || aux_file.length == 0 || /^\s+$/.test(aux_file) ){
		aux_file = "";
	}
	if(aux_id == "" && aux_file != ""){
		Biblioteca.notificaciones('El campo Nro Documento es requerido cuando Adjuntar documento está presente.', 'Plastiservi', 'error');
		event.preventDefault();
	}
	if(aux_id != "" && aux_file == ""){
		Biblioteca.notificaciones('El campo Adjuntar documento es requerido cuando Nro documento está presente.', 'Plastiservi', 'error');
		event.preventDefault();
	}
/*
	if($('#documento_file').attr('valor') == ''){
