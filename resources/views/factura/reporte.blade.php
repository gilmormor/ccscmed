		...
		<tr>
			...
			<td class="info_factura">
				<div>
					...
					<p>Nro Interno: <strong> {{ str_pad($despachoord->guiadesp->id, 10, "0", STR_PAD_LEFT) }}</strong>
						@if ($despachoord->despachoordanul)
							<small class="btn btn-danger btn-xs">Anulado</small>
						@endif
					</p>
					<p>Fecha Act: {{date('d-m-Y h:i:s A')}}</p>
					<p>Fecha: {{date('d-m-Y', strtotime($despachoord->guiadesp->fechahora))}}</p>
					<p>Hora: {{date("h:i:s A", strtotime($despachoord->guiadesp->fechahora))}}</p>
	...
	<div class="round1">
		...
		<table id="factura_detalle">
			...
			<tr>
				...
				<td class="textleft" width="50%"><span>{{str_pad(($despachoord->guiadesp ? $despachoord->guiadesp->nrodocto : "") , 10, "0", STR_PAD_LEFT)}}</span></td>
			</tr>
			<tr>
